...
export default class ProductsController {
    static async getProductsByClientCode(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!clientCode) {
                return res.status(HTTP_BAD_REQUEST).json({ error: ERROR_MESSAGES.CLIENT_CODE_IS_REQUIRED });
            }

            const products = await productService.getProductsByClient(clientCode);
            return res.status(HTTP_OK).json(products);
        } catch (error) {
            return res.status(HTTP_INTERNAL_SERVER_ERROR).json({ error: c.FAILED_TO_RETRIEVE_PRODUCTS_FOR_CLIENT });
        }
    }
    ...
    static async getAllProducts(_req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!products || !Array.isArray(products)) {
                return res.status(HTTP_BAD_REQUEST).json({ error: ERROR_MESSAGES.INVALID_PRODUCT_DATA });
            }
            return res.status(HTTP_OK).json(products);
        } catch (error) {
    ...
    static async getProductsByClient(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const products = await productService.getProductsByClient(client.clientCode);
            return res.status(HTTP_OK).json({ data: products });
        } catch (error) {
            return res.status(HTTP_BAD_REQUEST).json({ error: c.FAILED_TO_RETRIEVE_PRODUCTS_FOR_CLIENT });
        }
}
